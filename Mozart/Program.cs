    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            int[,] minuet = {
                ...
                {  98, 142,  42, 156,  75, 128,  62, 123,  65,  77,  19,  82, 137,  38, 149,   8 },
            ...
            {
                // throw 2 dices to determine which piece to play
                //NOTE: not equivalent to `random.Next(0, 11)`
                int column = random.Next(0, 6) + random.Next(0, 6);
                //int column = random.Next(0, 11)

                soundplayer.SoundLocation = $@"audio\M{minuet[column, i]}.wav";
                soundplayer.Load();
                Console.WriteLine($"{i + 1}: M{minuet[column, i]}.wav");
            ...
            {
                // throw 1 dice to determine which piece to play
                int column = random.Next(0, 6);

                soundplayer.SoundLocation = $@"audio\T{trio[column, i]}.wav";
                soundplayer.Load();
                Console.WriteLine($"{i + 17}: T{trio[column, i]}.wav");
                soundplayer.PlaySync();
            }
        }
    }
